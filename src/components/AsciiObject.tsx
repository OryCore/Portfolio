...
export interface AsciiObjectOptions {
  ...
  invert?: boolean;
  /** Flip character lightness while keeping hue, so a bright object reads as dark ink. Use on light backgrounds. */
  invertColor?: boolean;
...
function buildShapes(canvas: HTMLCanvasElement, aspectW: number, aspectH: number) {
  const rectangle = () => new THREE.Shape([new THREE.Vector2(0, 0), new THREE.Vector2(aspectW, 0), new THREE.Vector2(aspectW, aspectH), new THREE.Vector2(0, aspectH)]);
...
export function createAsciiObject(elements: AsciiObjectElements, options: AsciiObjectOptions = {}): AsciiObjectInstance | null {
  ...
  function applyOptions() {
    ...
    postMaterial.uniforms.uInvertColor.value = config.invertColor ? 1 : 0;
